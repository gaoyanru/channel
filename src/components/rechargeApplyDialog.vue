  ...
  <el-dialog title="充值申请" :visible.sync="dialogFormVisible" size="small">
    <el-form :model='ruleForm' ref="ruleForm" class="demo-ruleForm recharge-dialog" label-width="100px">
      <el-form-item v-if="!isReadOnly" label="充值金额：" required>
        ...
        <el-button class="btn btn-amount" :class="{'el-icon-check': rAmount != 5000 && rAmount != 10000}" @click="rAmount = ''; ismore = true">更多</el-button>
        <input class="input-style" v-show="ismore" v-model="rAmount" debounce="500" type="text" @change="rAmount = Math.round(rAmount*100)/100 || ''">
      </el-form-item>
      <el-form-item v-if="isReadOnly" label="充值金额：" required>
